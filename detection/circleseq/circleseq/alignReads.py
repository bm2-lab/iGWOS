...
def alignReads(BWA_path, HG19_path, read1, read2, outfile):
    ...
    samtools_sam_to_bam_command_0 = 'samtools view -bS {1} -o {0}'.format(bam_filename, sam_filename)
    samtools_sam_to_bam_command_1 = 'samtools sort {0} -o {1}'.format(bam_filename, base_name+'_sorted.bam')
    samtools_index_command = 'samtools index {0}'.format(base_name+'_sorted.bam')
    samtools_sort_by_name_command = 'samtools sort -o {0} -n {1}'.format(base_name+'_sorted.bam', bam_filename)
    ...
    logger.info(samtools_sam_to_bam_command_0)
    subprocess.check_call(samtools_sam_to_bam_command_0, shell=True)
    logger.info('Sorting by coordinate position for {0} complete.'.format(sample_name))

    logger.info(samtools_sam_to_bam_command_1)
    subprocess.check_call(samtools_sam_to_bam_command_1, shell=True)
    logger.info('Sorting by coordinate position for {0} complete.'.format(sample_name))
