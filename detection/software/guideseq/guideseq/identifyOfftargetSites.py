...
def analyze(sam_filename, reference_genome, outfile, annotations):
    ...
    with open(outfile, 'w') as f:
        # Write header
        f.write('\t'.join(['#BED_Chromosome', 'BED_Min.Position',
                           'BED_Max.Position', 'BED_Name', 'Filename', 'WindowIndex', 'Chromosome', 'Position', 'Sequence', '+.mi', '-.mi', 'bi.sum.mi', 'bi.geometric_mean.mi', '+.total',
                           '-.total', 'total.sum', 'total.geometric_mean', 'primer1.mi', 'primer2.mi', 'primer.geometric_mean',
                           'position.stdev', 'Off-Target_Sequence', 'Mismatches', 'Length', 'BED_off-target_Chromosome', 'BED_off-target_start', 'BED_off-target_end', 'BED_off-target_name', 'BED_Score', 'Strand', 'Cells', 'Targetsite', 'Target_Sequence']) + '\n')
