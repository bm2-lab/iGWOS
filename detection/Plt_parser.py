...
def gen_arg():
    ...
    subparsers =parser.add_subparsers(title='subcommands', help = 'the experimental method of the data', dest='type')
    """
    ...
    parser.add_argument('-L', metavar='label', type=str, nargs='?',
                        dest='label',
                        help='the name of sgRNA which you use in the experiment')
    parser.add_argument('-r1', metavar='read1', type=str, nargs='?',
                        dest='read1',
                        help='treated sequencing data 1, it should be noticed that different methods require different types of input data')
    parser.add_argument('-r2', metavar='read2', type=str, nargs='?',
                        dest='read2',
                        default='',
                        help='treated sequencing data 2, if sequencing data is single-end, only read1 is necessary')
    parser.add_argument('-R', metavar='target', type=str, nargs='?',
                        dest='target',
                        help='target sequence including PAM')
    parser.add_argument('-F', metavar='reference', type=str,nargs='?',
                        dest='reference',
                        help="reference genome file, make sure the genome index files under the same path when you choose SITE-seq")
    ...
    parser_a.add_argument('--d-minreads', metavar='demultiplex_min_reads', type=int, nargs='?',
                        ...
                        default=1000,
                        help='demultiplex_min_reads, necessary in GUIDE-seq with default value 1000')
    ...
    parser_b.add_argument('-rt', metavar='read_threshold', type=int, nargs='?',
                        dest='read_threshold',
                        default=4,
                        help='minimum number of reads at a location for that location to be called as a site, necessary in CIRCLE-seq with default value 4')
    parser_b.add_argument('-ws', metavar='window_size', type=int, nargs='?',
                        dest='window_size',
                        default=3,
                        help='size of the sliding window, necessary in CIRCLE-seq with default value 3')
    parser_b.add_argument('-mqt', metavar='mapq_threshold', type=int, nargs='?',
                        dest='mapq_threshold',
                        default=50,
                        help='minimum read mapping quality score, necessary in CIRCLE-seq with default value 50')
    parser_b.add_argument('-st', metavar='start_threshold', type=int, nargs='?',
                        dest='start_threshold',
                        default=1,
                        help='tolerance for breakpoint location, necessary in CIRCLE-seq with default value 1')
    parser_b.add_argument('-gt', metavar='gap_threshold', type=int, nargs='?',
                        dest='gap_threshold',
                        default=3,
                        help='number of tolerated gaps in the fuzzy target search step, necessary in CIRCLE-seq with default value 3')
    parser_b.add_argument('-mt', metavar='mismatch_threshold', type=int, nargs='?',
                        dest='mismatch_threshold',
                        default=6,
                        help='number of tolerated gaps in the fuzzy target search step, necessary in CIRCLE-seq with default value 6')
    parser_b.add_argument('-ma', metavar='merged_analysis', type=bool, nargs='?',choices=[True,False],
                        dest='merged_analysis',
                        default=True,
                        help='whether or not the paired read merging step should be performed, necessary in CIRCLE-seq with default value True')

    parser_c = subparsers.add_parser('SITE-seq',help = 'selective enrichment and identification of tagged genomic DNA ends by sequencing. It should be noticed that the data provided by the paper has some problems and the data is not in accord with the mechanism of SITE-seq. So we can not test the correction of this part')
...
def check_parser(args):
    ...
    args_str = str(args).split('(')[1].split(')')[0].split(',')
    args_str = [i.split('=')[1].strip("'") for i in args_str]
    input_file = []
    for i in args_str:
        v = [1 for j in ('.gz','.fq','fastq','.fa','.fasta','.txt') if j in i]
        if sum(v) > 0:
            input_file.append(i)
    n = 0
    non_exists = []
