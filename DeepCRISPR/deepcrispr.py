...
def build_ontar_reg_model(inputs_sg, scope='ontar'):
    with tf.variable_scope(scope):
        ...
        for i in range(1, len(channel_size) - 1):
            ...
            u = encoder_bn_u[i](pre_u, False, test_local_stats)
        ...
        u_last = encoder_bn_u[-1](pre_u_last, False, test_local_stats)
        ...
        for i in range(1, len(cls_channel_size)):
            ...
            l = cls_bn_layers[i](pre_l, False, test_local_stats)
            hl = tf.nn.relu(l)
            l_lst.append(l)
            hl_lst.append(hl)
...
def build_offtar_reg_model(inputs_sg, inputs_ot, scope='offtar'):
    with tf.variable_scope(scope):
        ...
        for i in range(1, len(channel_size) - 1):
            ...
            u_sg = encoder_bn_u_sg[i](pre_u_sg, False, test_local_stats)
            ...
            u_ot = encoder_bn_u_ot[i](pre_u_ot, False, test_local_stats)
        ...
        u_last_sg = encoder_bn_u_sg[-1](pre_u_last_sg, False, test_local_stats)
        ...
        u_last_ot = encoder_bn_u_ot[-1](pre_u_last_ot, False, test_local_stats)
